...
import { Users2, Plus, Calendar, CreditCard, Search, UserCheck, Trash2 } from 'lucide-react';
    ...
    <div className="space-y-6 animate-in fade-in duration-200">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-xs text-slate-400">View staff positions, record daily attendances, execute monthly base salary disbursements, and schedule shift operations.</p>
        </div>
      ...
      <div className="flex border-b border-indigo-500/10 pb-0.5 space-x-1">
        <button
          onClick={() => setActiveTab('employees')}
          className={`px-4 py-2.5 rounded-xl text-xs font-bold uppercase tracking-wider flex items-center gap-2 transition-all duration-150 border-b-2 ${
            activeTab === 'employees' 
              ? 'bg-indigo-600/10 text-indigo-400 border-indigo-500' 
              : 'hover:bg-slate-800/40 text-slate-400 border-transparent hover:text-slate-200'
          }`}
        >
          <Users2 className="w-4 h-4 shrink-0" />
          <span>{lang === 'en' ? 'Employee Directory & Payroll' : 'បញ្ជីបុគ្គលិក និងប្រាក់បៀវត្សរ៍'}</span>
        </button>
        <button
        ...
        >
          ...
          <span>{lang === 'en' ? 'Shift Schedule Calendar' : 'ប្រតិទិនវេនការងារ'}</span>
        </button>
      </div>

      {/* RENDER EMPLOYEE LIST DIRECTORY VIEW */}
      {activeTab === 'employees' ? (
        ...
        /* RENDER MODULAR SCHEDULE CALENDAR VIEW */
        <ShiftCalendar 
          staff={staff}
          lang={lang}
          t={t}
      ...
      {/* Styled Add Modal */}
      {showAddModal && (
        <div className="fixed inset-0 z-50 overflow-y-auto flex items-center justify-center bg-transparent backdrop-blur-md px-4 text-xs">
          <div className="bg-slate-800 border border-slate-700 rounded-2xl max-w-sm w-full p-6 shadow-2xl relative">
            ...
            <form onSubmit={handleAddStaffSubmit} className="space-y-4">
              <div>
                <label className="block text-[11px] font-semibold text-slate-400 mb-1">{t('staffName')} *</label>
                <input 
                  type="text" 
                  value={name}
              ...
              <div>
                <label className="block text-[11px] font-semibold text-slate-400 mb-1">{t('position')}</label>
                <select
                  value={position}
                  onChange={(e) => setPosition(e.target.value)}
                  className="w-full bg-slate-900 border border-slate-700 rounded-lg px-3 py-2 text-white outline-none cursor-pointer"
                >
              ...
              <div>
                <label className="block text-[11px] font-semibold text-slate-400 mb-1">Monthly Wage ($) *</label>
                <input 
                  type="number" 
                  value={salary}
              ...
              <div className="pt-4 border-t border-slate-700 flex justify-end gap-2.5">
                <button 
                  ...
                  className="px-3.5 py-2 bg-slate-700 hover:bg-slate-650 rounded-lg text-slate-300 font-semibold"
                >
                  {t('cancel')}
                </button>
                <button 
